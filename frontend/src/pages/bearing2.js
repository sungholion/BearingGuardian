...
export default function Bearing2Page() {
  const [isCollapsed, setIsCollapsed] = useState(false);
  const { theme } = useTheme();

  const [rulValue, setRulValue] = useState(55);
  const [remainingCycles, setRemainingCycles] = useState(55 * 8640);
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, padding: '0 32px 32px 32px', display: 'flex', gap: 24 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={cardStyle}>
              <BearingInfo bearingId="bearing2" />
            </div>
            <div style={cardStyle}>
              <FrequencyAnalysis />
            </div>
