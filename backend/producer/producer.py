# Kafka Producer for Bearing Data Simulation
# This script simulates bearing sensor data and sends it to a Kafka topic.
...
# Global counters for event ratios (for simulating different defect types)
event_counts = {"normal": 0, "outer": 0, "inner": 0}
total_events = 0

# Define parameter ranges for normal bearing operation
NORMAL_PARAMS = {
...
# These parameters would typically be derived from real-world defect characteristics.
DEFECT_PARAMS = {
...
DEFECT_CHANCE = 0.1 # 10% chance of a defect occurring (for future use, currently controlled by a_race)

def generate_data():
    # Initialize defect parameters to zero (no defect by default)
    a_L = 0.0
    a_N = 0
    a_lambda = [0.0] * 5
    a_delta = [0.0] * 5

    # Generate normal bearing parameters within their defined ranges
    ...
    # This logic ensures the ratio is maintained over every 100 events for realistic simulation.
    global event_counts, total_events

    if total_events % 100 == 0:
        # Reset counts for the new block of 100 events to maintain ratios
        event_counts = {"normal": 0, "outer": 0, "inner": 0}
    ...
    if possible_races:
        ...
        # Fallback if no possible races are left (should not happen with correct logic)
        a_race = random.choice(["normal", "outer", "inner"])

    total_events += 1

    # Introduce defect parameters only if a_race is not "normal"
    # This simulates the presence of a defect when the bearing is not normal.
    if a_race != "normal":
    ...
    # Prepare simulation input data structure
...
try:
    # Main loop to continuously generate and send data
    while True:
        data = generate_data()
